...
import { PARETO_CLASS_LABELS, ParetoClassTag } from '@/state/ducks/pareto/constants';
import { useParetoSelector } from '@/state/ducks/pareto/selectors';
import { paretoRequest } from '@/state/ducks/pareto/slice';
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const variantMap: Record<ParetoClassTag, 'default' | 'process' | 'destructive'> = {
        [ParetoClassTag.A]: 'default',
        [ParetoClassTag.B]: 'process',
        [ParetoClassTag.C]: 'destructive',
      };
...
const ParetoProductReportPage: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(paretoRequest());
  }, [dispatch]);

  const rows: Pareto[] = paretoSelector.data.paretos || [];
  const classifications: ParetoClassification[] = paretoSelector.data.classifications || [];
          ...
          <div className={cn('grid gap-3', isMobile ? 'grid-cols-1' : 'sm:grid-cols-3')}>
            {classifications.map((item) => (
              ...
              >
                ...
                <p className="mt-2 text-2xl font-bold">{item.tagPercentage}%</p>
                <p className="text-xs">
                  Value share: <span className="font-semibold">{item.valuePercentage}%</span>
                </p>
              </div>
            ))}
          </div>
